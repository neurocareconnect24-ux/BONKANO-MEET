...
Future<void> reGenerateToken() async {
  ...
  if (loginUserData.value.isSocialLoginType) {
    ...
    // Tenter la ré-authentification avec le mot de passe stocké (legacy)
    // ou forcer la déconnexion si non disponible
    final userPASSWORD = getValueFromLocal(SharedPreferenceConst.USER_PASSWORD);
    if (userPASSWORD is String && userPASSWORD.isNotEmpty) {
      req[UserKeys.password] = userPASSWORD;
    } else {
      // Pas de mot de passe stocké - forcer la déconnexion
      log('No stored password available for token regeneration, forcing logout');
    ...
    if (files != null && files.isNotEmpty) {
      ...
      if (files.length > 1) {
        ...
        if (isKeyRequireIndexing) {
          multiPartRequest.files.add(await MultipartFile.fromPath('${fileKey}_0', element.path));
        } else {
...
void apiPrint({
  ...
  // Ne pas loguer les détails API en mode release (sécurité)
