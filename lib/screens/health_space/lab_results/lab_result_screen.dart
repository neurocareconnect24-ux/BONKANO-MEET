...
class LabResultScreen extends StatelessWidget {
  LabResultScreen({super.key});

  final LabResultController controller = Get.put(LabResultController());

  static const Color _iconColor = Color(0xFF48D0B8);

  @override
  Widget build(BuildContext context) {
    return AppScaffold(
      appBartitleText: locale.value.labResults,
                    ...
                    return Container(
                      ...
                      child: ListTile(
                        ...
                        leading: Container(
                          ...
                          child: const Icon(Icons.science_rounded, color: _iconColor, size: 22),
                        ),
                        title: Text(item.testName, style: boldTextStyle(size: 14)),
                        subtitle: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            4.height,
                            if (item.date.isNotEmpty) Text(item.date, style: secondaryTextStyle(size: 12)),
                            if (item.result.isNotEmpty) Text('${item.result}${item.unit.isNotEmpty ? ' ${item.unit}' : ''}', style: secondaryTextStyle(size: 12)),
                            if (item.normalRange.isNotEmpty) Text('${locale.value.normalRange}: ${item.normalRange}', style: secondaryTextStyle(size: 12)),
                            if (item.labName.isNotEmpty) Text(item.labName, style: secondaryTextStyle(size: 12)),
                          ],
                        ),
                        trailing: IconButton(
      ...
      fabWidget: FloatingActionButton(
        ...
        onPressed: () => Get.to(() => AddLabResultScreen()),
  ...
  Widget _emptyState() {
    return Center(
      child: Column(
        ...
        children: [
          const Icon(Icons.science_rounded, size: 60, color: secondaryTextColor),
          16.height,
