...
class VideoCallController extends GetxController {
  ...
  Future<void> _initCall() async {
    ...
    log('=== AGORA DEBUG [Patient] === Step 1: Requesting permissions...');
    bool permGranted = await _handlePermissions();
    if (!permGranted) {
      log('=== AGORA DEBUG [Patient] === FAILED: Permissions denied');
      callState(VideoCallState.failed);
      return;
    }
    log('=== AGORA DEBUG [Patient] === Step 1: Permissions granted OK');

    // 2. Fetch token from backend
    try {
      log('=== AGORA DEBUG [Patient] === Step 2: Fetching token for appointment ${appointment.id}...');
      ...
      _uid = tokenRes.uid;
      log('=== AGORA DEBUG [Patient] === Step 2: Token received OK');
      log('=== AGORA DEBUG [Patient] === token length: ${_token.length}, channel: $_channelName, uid: $_uid');
      if (_token.isEmpty || _channelName.isEmpty) {
        log('=== AGORA DEBUG [Patient] === FAILED: Token or channel name is EMPTY!');
        toast(locale.value.failedToJoinCall);
        callState(VideoCallState.failed);
        return;
      }
    } catch (e) {
      log('=== AGORA DEBUG [Patient] === FAILED Step 2: Token error: $e');
      toast(locale.value.failedToJoinCall);
      callState(VideoCallState.failed);
      return;
    }
    ...
    try {
      log('=== AGORA DEBUG [Patient] === Step 3: Creating Agora engine with appId: $AGORA_APP_ID');
      agoraEngine = createAgoraRtcEngine();
      await agoraEngine!.initialize(RtcEngineContext(
        appId: AGORA_APP_ID,
        channelProfile: ChannelProfileType.channelProfileCommunication,
      ));
      log('=== AGORA DEBUG [Patient] === Step 3: Engine initialized OK');
    } catch (e) {
      log('=== AGORA DEBUG [Patient] === FAILED Step 3: Engine init error: $e');
      toast(locale.value.failedToJoinCall);
      callState(VideoCallState.failed);
      return;
    }
    ...
    agoraEngine!.registerEventHandler(RtcEngineEventHandler(
      onJoinChannelSuccess: (RtcConnection connection, int elapsed) {
        log('=== AGORA DEBUG [Patient] === onJoinChannelSuccess: channel=${connection.channelId}, uid=${connection.localUid}, elapsed=$elapsed ms');
        callState(VideoCallState.connected);
      },
      onUserJoined: (RtcConnection connection, int rUid, int elapsed) {
        log('=== AGORA DEBUG [Patient] === onUserJoined: remoteUid=$rUid');
        remoteUid(rUid);
        isRemoteUserJoined(true);
        toast(locale.value.doctorJoinedTheCall);
        _startTimer();
      },
      onUserOffline: (RtcConnection connection, int rUid, UserOfflineReasonType reason) {
        log('=== AGORA DEBUG [Patient] === onUserOffline: remoteUid=$rUid, reason=$reason');
        remoteUid(0);
        isRemoteUserJoined(false);
        toast(locale.value.doctorLeftTheCall);
        _stopTimer();
      },
      onConnectionStateChanged: (RtcConnection connection, ConnectionStateType state, ConnectionChangedReasonType reason) {
        log('=== AGORA DEBUG [Patient] === onConnectionStateChanged: state=$state, reason=$reason');
      },
      onConnectionLost: (RtcConnection connection) {
        log('=== AGORA DEBUG [Patient] === onConnectionLost');
        callState(VideoCallState.disconnected);
        toast(locale.value.callDisconnected);
      },
      onTokenPrivilegeWillExpire: (RtcConnection connection, String token) {
        log('=== AGORA DEBUG [Patient] === onTokenPrivilegeWillExpire - token is about to expire!');
      },
      onError: (ErrorCodeType err, String msg) {
        log('=== AGORA DEBUG [Patient] === onError: code=$err, msg=$msg');
      },
    ));

    // 5. Enable audio + video and join channel
    try {
      log('=== AGORA DEBUG [Patient] === Step 5: enableAudio...');
      await agoraEngine!.enableAudio();
      log('=== AGORA DEBUG [Patient] === Step 5: enableVideo...');
      await agoraEngine!.enableVideo();
      log('=== AGORA DEBUG [Patient] === Step 5: startPreview...');
      await agoraEngine!.startPreview();
      log('=== AGORA DEBUG [Patient] === Step 5: joinChannel(channel=$_channelName, uid=$_uid, tokenLen=${_token.length})...');
      ...
      log('=== AGORA DEBUG [Patient] === Step 5: joinChannel call completed (waiting for callback)...');
    } catch (e) {
      log('=== AGORA DEBUG [Patient] === FAILED Step 5: $e');
      toast(locale.value.failedToJoinCall);
      callState(VideoCallState.failed);
    }
  }
