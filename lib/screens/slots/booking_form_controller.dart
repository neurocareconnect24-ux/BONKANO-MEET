...
import 'package:bonkano_meet/utils/common_base.dart';
...
import 'model/available_days_res_model.dart';
...
class BookingFormController extends GetxController {
  ...
  Rx<AvailableDaysResModel?> availableDaysData = Rx<AvailableDaysResModel?>(null);
  RxList<DateTime> inactiveDatesList = RxList<DateTime>();
  ...
  Future<void> fetchAvailableDays() async {
    if (selectedClinic.value.id.isNegative || selectedService.value.id.isNegative || selectedDoctor.value.doctorId.isNegative) {
      return;
    }
    
    try {
      final res = await CoreServiceApis.getAvailableDays(
        clinicId: selectedClinic.value.id,
        serviceId: selectedService.value.id,
        doctorId: selectedDoctor.value.doctorId,
      );
      availableDaysData(res);
      calculateInactiveDates();
    } catch (e) {
      log("fetchAvailableDays error $e");
    }
  }

  void calculateInactiveDates() {
    if (availableDaysData.value == null) return;
    
    inactiveDatesList.clear();
    
    DateTime startDate = DateTime.now().subtract(const Duration(days: 1)); // start slightly in past to cover all current
    for (int i = 0; i < 90; i++) { // Generate inactive dates for next 90 days
      DateTime checkDate = startDate.add(Duration(days: i));
      String dateString = checkDate.formatDateYYYYmmdd();
      
      bool isInactive = false;
      
      int apiWeekday = checkDate.weekday % 7;
      if (availableDaysData.value?.disabledWeekdays?.contains(apiWeekday) ?? false) {
        isInactive = true;
      }
      
      if (availableDaysData.value?.full?.contains(dateString) ?? false) {
        isInactive = true;
      }
      
      if (availableDaysData.value?.available != null && availableDaysData.value!.available!.isNotEmpty) {
        if (!availableDaysData.value!.available!.contains(dateString)) {
           isInactive = true;
        }
      }
      
      if (isInactive) {
        inactiveDatesList.add(checkDate);
      }
    }
  }

  void onDateTimeChange() {
    final appointmentDateTime = "${selectedDate.value} ${selectedSlot.value.amPMto24HourFormat}";
    if (appointmentDateTime.isValidDateTime || (selectedDate.value.isNotEmpty && selectedSlot.value.isNotEmpty)) {
      nextBtnVisible(true);
    } else {
